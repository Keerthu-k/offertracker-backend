...
def get_dashboard(
    *,
    ...
    try:
        ...
        logger.error("Dashboard computation failed for user %s: %s", user_id, exc)
        raise HTTPException(status_code=500, detail="Failed to compute analytics dashboard")
...
def get_activity_log(
    *,
    ...
    try:
        ...
        logger.error("Failed to load activity log for user %s: %s", current_user["id"], exc)
...
def get_interview_questions(
    *,
    ...
    try:
        ...
        logger.error("Failed to load interview questions for user %s: %s", user_id, exc)
